...
class Excercises_Item extends StatelessWidget {
  String text;
  Excercises_Item({Key key, @required this.text}) : super(key: key);

  Http_dio.Dio dio = Http_dio.Dio();
  Future<List<ExcercisesModel>> _getExcercises() async {
    Http_dio.Response response = await dio
        .get("http://fitandheal.somee.com/api/BtapCalo/GetByidMucDich/1");

    if (response.statusCode == 200) {
      final List _excercisesData = jsonDecode(jsonEncode(response.data));
      List<ExcercisesModel> _listExcercises =
          _excercisesData.map((e) => ExcercisesModel.fromJson(e)).toList();
      return _listExcercises;
    } else {
      return null;
    }
  }
